...
document.addEventListener("DOMContentLoaded", function () {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/mange/allUsers", {
        ...
        headers: {
          ...
          Authorization:
            "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2NGM5MWNjOGZhMzNlZTlmNDFmYTg4ODMiLCJpYXQiOjE2OTA5MDE3MDQsImV4cCI6MTcyNjkwMTcwNH0.QpudSuJlfwDjYtXY-wobDjxFs-jJIGB94mBJRZ1h47w",
        },
      });
      if (response.ok) {
    ...
    if (editBtn) {
      ...
      try {
        const response = await fetch(
          `http://localhost:3000/api/mange/change/${index}`,
          {
            method: "put",
            headers: {
  ...
  booksList.addEventListener("click", (e) => {
    ...
    if (deleteBtn) {
      ...
      const confirmDelete = confirm(
        `Are you sure you want to delete this book?  ${book.name}`
      );
      if (confirmDelete) {
        deleteBook(id);
      }
  ...
  async function deleteBook(id) {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/mange/books/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json", // Add the Content-Type header to indicate JSON data
  ...
  editBookForm.addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/mange/books/${_id}`,
        {
          method: "Put",
          headers: {
            "Content-Type": "application/json", // Add the Content-Type header to indicate JSON data
  ...
  function searchAndFilterData(searchTerm) {
    ...
    booksArray.forEach((book) => {
      if (
        fuzzyMatch(book.title, searchTerm) ||
        fuzzyMatch(book.author, searchTerm) ||
        fuzzyMatch(book.genre, searchTerm)
      ) {
    ...
    usersArray.forEach((user) => {
      if (
        fuzzyMatch(user.name, searchTerm) ||
        fuzzyMatch(user.email, searchTerm)
      ) {
    ...
    adminsArray.forEach((admin) => {
      if (
        fuzzyMatch(admin.name, searchTerm) ||
        fuzzyMatch(admin.email, searchTerm)
      ) {
